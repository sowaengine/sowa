...
void UICanvas::Text(const std::string &text, Font *font, float scale) {
	...
	App::GetInstance().Renderer().DrawText(text, font, container->PosX() + container->CursorX() + m_padding, container->PosY() + container->CursorY() - m_lineSize - m_padding, glm::mat4(1.f), 0.f, scale, maxSize.x, maxSize.y);
