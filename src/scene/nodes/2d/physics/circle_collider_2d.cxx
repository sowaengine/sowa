#include "circle_collider_2d.hxx"
...
void *CircleCollider2D::body_add_fixture(void *body) {
	return PhysicsServer2D::get().body_add_circle_shape(this, body, id(), radius(), position());
}

void CircleCollider2D::_update() {

	glm::mat4 model = calculate_transform();
	model = glm::scale(model, {radius() * 2, radius() * 2, 1.f});
