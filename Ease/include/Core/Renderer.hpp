...
class Renderer {
  public:
	...
	void DrawTexture(const glm::vec2 &position, const glm::vec2 &scale, float zIndex, float rotation, Ease::ImageTexture &texture, uint32_t id = 0);
	// void DrawQuad(const glm::vec2& position, const glm::vec2& scale, float zIndex, float rotation, ImageTexture& texture, const glm::vec2& uv1, const glm::vec2& uv2);
	// void DrawQuadWithID(const glm::vec2& position, const glm::vec2& scale, float zIndex, float rotation, ImageTexture& texture, uint32_t id, const glm::vec2& uv1, const glm::vec2& uv2);
